...
def is_available(equipment, from_date, to_date, ignore_contract=None,
                 ignore_reservation=None):
    from equip_rental.utils.cross_hire import get_hire_window
    ...
    if window:
        if window.hire_available_from and getdate(from_date) < getdate(
                window.hire_available_from):
            return False
        if window.hire_available_upto and getdate(to_date or from_date) > getdate(
                window.hire_available_upto):
            return False
...
def validate_availability(equipment, from_date, to_date, ignore_contract=None):
    from equip_rental.utils.cross_hire import validate_hire_window

    validate_hire_window(equipment, from_date, to_date)
